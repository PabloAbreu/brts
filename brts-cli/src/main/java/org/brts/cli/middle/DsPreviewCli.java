...
public class DsPreviewCli {
    ...
    static class Options {
        @Option(name = "--input", required = true,
                usage = "Path to the .m2ts file containing an IGS stream")
        File input;

        @Option(name = "--ds-index",
                usage = "Zero-based index of the display set to preview (default: 0)")
        int displaySetIndex = 0;
    }

    public static class Run extends FeatureRunner<Options> {
        @Override public String getCommandName() { return "ds-preview"; }
        @Override public String getDescription() { return "Interactive Swing preview for IGS menus in M2TS"; }
        ...
        protected void execute(Options opts) throws Exception {
            ...
            System.out.println("Loaded: " + model.getScreenWidth() + "×" + model.getScreenHeight()
                    + ", " + model.getPages().size() + " page(s), "
                    + model.getObjectImages().size() + " decoded object(s).");

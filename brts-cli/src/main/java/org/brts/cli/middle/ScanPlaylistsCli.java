...
public class ScanPlaylistsCli {
    ...
    static class Options {
        ...
        @Option(name = "--type",
                usage = "Force disc content type: MOVIE or TV_SERIES (default: auto-detect)")
        String forcedType;

        @Option(name = "--movie-min",
                usage = "Minimum duration in minutes for a playlist to be a movie (default: 80)")
        ...
        @Option(name = "--include-menus",
                usage = "Include menu playlists in results (default: excluded)")
        boolean includeMenus = false;
    }

    public static class Run extends FeatureRunner<Options> {
        @Override public String getCommandName() { return "scan-playlists"; }
        @Override public String getDescription() { return "Scan Blu-ray playlists and auto-detect content type"; }
        ...
        protected void execute(Options opts) throws Exception {
            ...
            if (opts.movieMinMinutes != null)       cb.movieMinMinutes(opts.movieMinMinutes);
            if (opts.movieAlternateCutRatio != null) cb.movieAlternateCutRatio(opts.movieAlternateCutRatio);
            if (opts.episodeMinMinutes != null)      cb.episodeMinMinutes(opts.episodeMinMinutes);
            if (opts.episodeMaxDurationRatio != null) cb.episodeMaxDurationRatio(opts.episodeMaxDurationRatio);
            if (opts.episodeMinCount != null)        cb.episodeMinCount(opts.episodeMinCount);
            if (opts.includeMenus)                   cb.excludeMenus(false);

 ...
 * <p>
 * Each concrete subclass represents a single sub-command (e.g.
 * {@code clip-parse},
 * {@code m2ts-info}). It knows how to describe itself for usage output and how
 * to parse its own args4j options bean before delegating to {@link #execute}.
 *
...
public abstract class FeatureRunner<O> {
    ...
    /**
     * The sub-command name as typed on the command line (e.g.
     * {@code "clip-parse"}).
     */
    ...
    /**
     * Executes the feature logic after arguments have been parsed into
     * {@code opts}.
     */
    ...
    protected O createOptions() {
        ...
        while (clazz != null) {
            if (clazz.getSuperclass() == FeatureRunner.class) {
                // found the direct subclass of FeatureRunner, now determine the type parameter
                // O
    ...
    /**
     * Parses command-line arguments into the options bean and invokes
     * {@link #execute}.
    ...
    /**
     * Prints the command name, description, and args4j option details to the
     * given stream.
     */
    ...
    /**
     * Writes {@code value} as JSON. If {@code output} is non-null, writes to that
     * file; otherwise pretty-prints to stdout.
     */

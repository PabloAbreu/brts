...
public class MkvToPlaylistCli {
    ...
    static class ConvertOptions {
        @Option(name = "--input", required = true,
                usage = "Path to the MKV source file")
        File input;
        ...
        @Option(name = "--clip-name",
                usage = "5-digit clip name (default: 00001)")
        String clipName = "00001";

        @Option(name = "--audio-tracks",
                usage = "Comma-separated MKV track numbers for audio (default: all)")
        String audioTracks;

        @Option(name = "--subtitle-tracks",
                usage = "Comma-separated MKV track numbers for subtitles (default: all)")
        String subtitleTracks;

        @Option(name = "--pgs-resolution",
                usage = "PGS render resolution as WIDTHxHEIGHT (default: 1920x1080)")
        String pgsResolution;

        @Option(name = "--pgs-font-name",
                usage = "Font for PGS subtitle rendering (default: SansSerif)")
        String pgsFontName;

        @Option(name = "--pgs-font-size",
                usage = "Font size for PGS subtitle rendering (default: 48)")
        Integer pgsFontSize;

        @Option(name = "--pgs-font-color",
                usage = "Text colour for PGS as RRGGBB hex (default: FFFFFF)")
        String pgsFontColor;

        @Option(name = "--pgs-outline-color",
                usage = "Outline colour for PGS as RRGGBB hex (default: 000000)")
        String pgsOutlineColor;
    }

    public static class Convert extends FeatureRunner<ConvertOptions> {
        @Override public String getCommandName() { return "mkv-to-playlist"; }
        @Override public String getDescription() { return "Convert an MKV file to Blu-ray M2TS/CLPI/MPLS"; }
        @Override protected ConvertOptions createOptions() { return new ConvertOptions(); }

        @Override
        protected void execute(ConvertOptions opts) throws Exception {
            MkvToPlaylistConverter.Config config = new MkvToPlaylistConverter.Config(
                    opts.input.toPath(),
                    opts.outputDir.toPath(),
                    opts.clipName
            );
            ...
            if (opts.pgsFontName != null) pgsConfig.setFontName(opts.pgsFontName);
            if (opts.pgsFontSize != null) pgsConfig.setFontSize(opts.pgsFontSize);

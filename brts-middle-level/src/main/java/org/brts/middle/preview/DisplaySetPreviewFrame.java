 ...
 * <p>
 * The window is freely resizable but enforces the original aspect ratio
 * (e.g. 16:9 for 1920×1080).  Arrow keys, Enter, and Page Up/Down are
 * used to navigate the menu, mimicking a Blu-ray remote control.
 * <p>
 * When a button is activated, the navigation commands associated with it
 * are briefly displayed as a semi-transparent overlay at the bottom of the
 * screen.
 *
 * <h2>Key bindings</h2>
 * <table>
 *   <tr><td>↑ / ↓ / ← / →</td><td>Move selection</td></tr>
 *   <tr><td>Enter / Space</td><td>Activate (confirm) selected button</td></tr>
 *   <tr><td>Page Up / Page Down</td><td>Switch page</td></tr>
...
public class DisplaySetPreviewFrame extends JFrame {
    ...
    private void onUp()       { nav.moveUp();    panel.repaint(); }
    private void onDown()     { nav.moveDown();   panel.repaint(); }
    private void onLeft()     { nav.moveLeft();   panel.repaint(); }
    private void onRight()    { nav.moveRight();  panel.repaint(); }
    ...
    /**
     * Opens the preview window on the EDT.
    ...
    public static void showAndWait(DisplaySetPreviewModel model) {
        log.info("Opening DS Preview: {}×{}, {} pages",
                model.getScreenWidth(), model.getScreenHeight(), model.getPages().size());

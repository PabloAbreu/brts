 ...
 * <p>
 * Handles directional movement (up / down / left / right), selection (Enter),
 * and numeric input.  After each action it updates the
 * {@link DisplaySetPreviewModel} and returns a {@link NavigationResult}
...
public class NavigationController {
    ...
    /**
     * Activate (enter / confirm) the currently selected button.
     * Checks navigation commands and returns a result describing them.
     */
    ...
    public NavigationResult goToPage(int pageId) {
        for (int i = 0; i < model.getPages().size(); i++) {
            if (model.getPages().get(i).getId() == pageId) {
                ...
                return new NavigationResult(NavigationResult.Type.PAGE_CHANGED, pageId,
                        "Page " + pageId);
            }
        }
    ...
    private NavigationResult moveToNeighbour(Direction dir) {
        ...
        return new NavigationResult(NavigationResult.Type.SELECTION_CHANGED, targetId,
                dir + " → button #" + targetId);
    }
    ...
    private boolean isButtonEnabled(IgsPage page, int buttonId) {
        for (int i = 0; i < page.getBogs().size(); i++) {
            ...
            for (IgsButton btn : page.getBogs().get(i).getButtons()) {
                if (btn.getId() == buttonId) return true;
            }
        }
        return false;
    }
    ...
    /**
     * Provides a human-readable description of a button's navigation commands.
     * This is displayed as an overlay when the user activates a button.
     * Delegates to {@link ParsedNavigationCommand} for proper mnemonic decoding.
     */

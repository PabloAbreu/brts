 ...
 *   <li>Produces a {@link ClipDescriptor} and a {@link PlaylistDescriptor}
 *       that can be fed to the low-level writers directly or written to JSON
 *       for manual inspection / re-use.</li>
 * </ol>
 */
public class SimpleTitleBuilder {
    ...
    public TitleBuildResult build(TitleDescriptor descriptor) throws IOException {
        ...
        for (SourceMediaInfo.SourceTrack track : mediaInfo.getTracks()) {
            ...
            if (track.getCodingType().isAudio() && !shouldIncludeByLanguage(track.getLanguage(), descriptor.getAudioLanguages())) continue;
            if (track.getCodingType().isSubtitle() && !shouldIncludeByLanguage(track.getLanguage(), descriptor.getSubtitleLanguages())) continue;
    ...
    public record TitleBuildResult(

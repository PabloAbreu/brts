...
/**
 * Factory that creates the appropriate {@link MediaSource} implementation
 * from a {@link BackgroundMediaDescriptor}.
 */
public final class MediaSourceFactory {
    ...
    public static MediaSource create(BackgroundMediaDescriptor descriptor) {
        if (descriptor instanceof MkvBackgroundMedia mkv) {
            return new MkvMediaSource(
                    Path.of(mkv.getFile()),
                    mkv.getVideoTrackNumber(),
                    mkv.getAudioTrackNumber()
            );
        }

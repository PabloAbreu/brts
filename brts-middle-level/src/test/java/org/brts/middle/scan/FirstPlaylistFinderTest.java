...
class FirstPlaylistFinderTest {
    ...
    /**
     * Creates a navigation command with both operands marked as immediate.
     * Suitable for PLAY_PL*, JUMP_OBJECT, JUMP_TITLE, CALL_OBJECT, CALL_TITLE
     * where operands are always literal values (playlist IDs, object indices, etc.).
     */
    ...
    /**
     * Creates a SET command where operand1 is a GPR reference (destination)
     * and operand2 is an immediate value (source).
     */
    ...
    private static IndexBdmv indexWith(TitleEntry firstPlay, TitleEntry topMenu,
    ...
    void directPlayPl_returnsPlaylistId() {
        ...
        MovieObjects mobj = mobjWith(
                movieObject(cmd("PLAY_PL", 100))
        );

        FirstPlaylistResult r = new FirstPlaylistFinder(index, mobj)
                .find(FirstPlaylistFinderConfig.builder().build());
    ...
    void directPlayPlPi_returnsPlaylistAndPlayItem() {
        IndexBdmv index = indexWith(hdmvTitle(0), null, List.of());
        MovieObjects mobj = mobjWith(
                movieObject(cmd("PLAY_PL_PI", 200, 3))
        );
    ...
    void directPlayPlPm_returnsPlaylistAndPlayMark() {
        IndexBdmv index = indexWith(hdmvTitle(0), null, List.of());
        MovieObjects mobj = mobjWith(
                movieObject(cmd("PLAY_PL_PM", 300, 5))
        );
    ...
    void jumpObject_chainsToTargetObject() {
        ...
        IndexBdmv index = indexWith(hdmvTitle(0), null, List.of());
        MovieObjects mobj = mobjWith(
                movieObject(cmd("JUMP_OBJECT", 1)),
                movieObject(cmd("PLAY_PL", 500))
        );
    ...
    void jumpTitle_resolvesViaTitleTable() {
        ...
        // Object 1: PLAY_PL 42
        IndexBdmv index = indexWith(hdmvTitle(0), null,
                List.of(hdmvTitle(1)));
        MovieObjects mobj = mobjWith(
                movieObject(cmd("JUMP_TITLE", 1)),
                movieObject(cmd("PLAY_PL", 42))
        );
    ...
    void jumpTitle_firstPlaySpecial() {
        ...
        MovieObjects mobj = mobjWith(
                movieObject(cmd("JUMP_TITLE", 0xFFFF)),
                movieObject(cmd("PLAY_PL", 88))
        );
    ...
    void callObject_resume_returnsToCallerObject() {
        ...
        // After RESUME, chainer goes back to object 0, re-runs with GPR state from RESUME.
        ...
        IndexBdmv index = indexWith(hdmvTitle(0), null, List.of());
        MovieObjects mobj = mobjWith(
                movieObject(cmd("CALL_OBJECT", 1)),
                movieObject(cmd("PLAY_PL", 333))
        );
    ...
    void callTitle_followsToTarget() {
        ...
        IndexBdmv index = indexWith(hdmvTitle(0), null,
                List.of(hdmvTitle(1)));
        MovieObjects mobj = mobjWith(
                movieObject(cmd("CALL_TITLE", 1)),
                movieObject(cmd("PLAY_PL", 777))
        );

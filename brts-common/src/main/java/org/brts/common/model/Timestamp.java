...
/**
 * Represents a Blu-ray 90 kHz timestamp (used in CLPI and MPLS).
 * The Blu-ray spec uses a 45 kHz clock for some fields and 90 kHz for others;
 * all values here are in 90 kHz units.
 */ 
public final class Timestamp {
    ...
    /**
     * MPLS PlayItem IN_time / OUT_time and PlayMark timestamps use the
     * 45 kHz STC clock, not the 90 kHz PTS clock.
     */
    ...
    public Timestamp(long ticks) {
        if (ticks < 0) throw new IllegalArgumentException("Timestamp ticks must be >= 0");
        this.ticks = ticks;
    }

...
/**
 * Reads big-endian binary data from an InputStream.
 * All Blu-ray binary structures use big-endian byte ordering.
 */
public class BinaryReader implements AutoCloseable {
    ...
    public int readUnsignedByte() throws IOException {
        int b = in.read();
        if (b < 0) throw new java.io.EOFException("Unexpected end of stream at position " + position);
        position++;
        return b;
    }
    ...
    public byte[] readBytes(int length) throws IOException {
        ...
        while (read < length) {
            ...
            if (n < 0) throw new java.io.EOFException("Unexpected end of stream at position " + position);
            read += n;
            position += n;
        }
        return buf;
    ...
    public void skip(long n) throws IOException {
        ...
        while (remaining > 0) {
            ...
            if (skipped <= 0) throw new java.io.EOFException("Unexpected end of stream at position " + position);
            position += skipped;
            remaining -= skipped;
        }
    }

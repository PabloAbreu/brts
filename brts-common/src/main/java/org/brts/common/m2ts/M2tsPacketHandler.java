 ...
 * <p>
 * Implementations decide where the raw elementary stream bytes go — to files
 * on disk, into an MKV container, or anywhere else. The demuxer
 * ({@link M2tsDemuxer}) invokes these methods once per relevant TS packet.
 *
 * <h2>Lifecycle</h2>
 * <ol>
...
public interface M2tsPacketHandler extends AutoCloseable {
     ...
     */
    void onPayload(int pid, byte[] payload, int offset, int length,
                   boolean payloadUnitStart, long packetIndex, long ats) throws IOException;

    /**
     * Release any resources held by this handler. Called once when demuxing
     * completes (successfully or after an error).
     */
    @Override

 ...
 * <p>
 * Produced by {@link org.brts.lowlevel.m2ts.M2tsParser} and serialisable to JSON
 * so CLI users can inspect a file without further tooling.
 */
@Getter
@Setter
public class M2tsInfo {
    ...
    /**
     * Total number of 192-byte Source Packets in the file
     * (i.e. file size / 192, rounded down).
     */
    private long totalPackets;

    /**
     * First arrival timestamp (ATS) seen in the TP_extra_header, in 27 MHz ticks.
     * A value of {@code -1} means no packets were read.
     */
    ...
    /**
     * First PCR (Program Clock Reference) value seen, in 27 MHz ticks.
     * {@code -1} if no PCR packet was found.
     */
    ...
    /**
     * Duration derived from first/last PCR, converted to milliseconds.
     * Returns {@code -1} if PCR data is not available.
     */
    public long getDurationMs() {
        if (firstPcr27MHz < 0 || lastPcr27MHz < 0) return -1;

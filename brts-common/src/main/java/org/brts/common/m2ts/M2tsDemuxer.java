 ...
 * <p>
 * Reads 192-byte source packets, strips the 4-byte {@code TP_extra_header}
 * and the TS header / adaptation field, and delivers raw PES-payload bytes
 * to an {@link M2tsPacketHandler}.
 * <p>
 * The demuxer itself is stateless with respect to <em>output</em> — all output
 * decisions are made by the handler. This allows the same demuxing logic to
 * drive file-per-PID extraction, MKV muxing, or any other consumer.
 *
 * <h2>Usage</h2>
 * 
 * <pre>{@code
...
public class M2tsDemuxer {
    ...
    /**
     * Demuxes the M2TS file at {@code source}, delivering payload bytes for
     * every packet whose PID is in {@code pidFilter} to the given
     * {@code handler}.
     *
     ...
     * @param pidFilter only these PIDs are delivered; pass {@code null} or
     *                  empty to deliver all PIDs present in {@code info}
     * @throws IOException on I/O error
     */
    public void demux(Path source, M2tsInfo info, M2tsPacketHandler handler,
            Set<Integer> pidFilter) throws IOException {
     ...
     * @param stopPacket  last packet to process (inclusive); pass {@code -1}
     *                    to process until end-of-file
     * @param keepTables  if true, PAT/PMT/PCR PIDs are included, unless {@code pidFilter} is set
     * @throws IOException on I/O error
     */
    public void demux(Path source, M2tsInfo info, M2tsPacketHandler handler,
            Set<Integer> pidFilter, long startPacket, long stopPacket, boolean keepTables)
            throws IOException {
        ...
        try (InputStream in = new BufferedInputStream(Files.newInputStream(source))) {
            ...
            while (packetIndex <= effectiveStop) {
                ...
                handler.onPayload(pid, sp, payloadOff, payloadLen,
                        payloadUnitStart, packetIndex, ats);

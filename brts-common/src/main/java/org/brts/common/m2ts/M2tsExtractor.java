 ...
 * <p>
 * For each PID listed in a parsed {@link M2tsInfo}, the extractor writes all
 * PES payload bytes for that PID into a file named
 * {@code <outputDir>/<pid>.<ext>}, where the extension is derived from the
 * stream type.
 * <p>
 * The output files are raw elementary streams (i.e. the MPEG-2 TS packetisation
 * and PES headers are removed). Downstream tools like FFmpeg can consume them
 * directly.
 * <p>
...
public class M2tsExtractor {
     ...
     * @param info      stream metadata (from {@link M2tsParser})
     * @param outputDir directory where elementary stream files will be written
     * @param pidFilter only these PIDs will be extracted; pass null or empty to
     *                  extract all
     * @throws IOException on I/O error
     */

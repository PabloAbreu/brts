 ...
 * <p>
 * Extracts track metadata (codec, resolution, frame rate, audio channels,
 * sample rate, language) from the MKV Tracks element without demuxing the frames.
 * <p>
...
public class MkvSourceMediaParser implements SourceMediaParser {
    ...
    public SourceMediaInfo parse(Path path) throws IOException {
        ...
        try (FileDataSource dataSource = new FileDataSource(path.toAbsolutePath().toString())) {
            ...
            // Duration: MatroskaFile.getDuration() returns duration in timecode scale units
            // (typically nanoseconds when timecodeScale=1000000, i.e. milliseconds * 1000000 ns)
            // getTimecodeScale() is in nanoseconds per tick, getDuration() is in ticks
            long timecodeScaleNs = mkv.getTimecodeScale(); // nanoseconds per tick (default 1,000,000)
            ...
            if (trackList != null) {
                for (MatroskaFileTrack mkvTrack : trackList) {
                    ...
                    if (trackType == TrackType.VIDEO) {
                        ...
                        log.debug("Video track {}: codec={}, {}×{}, fps={}", track.getTrackNumber(),
                                codecId, track.getWidthPixels(), track.getHeightPixels(), track.getFrameRateFps());
                        ...
                        log.debug("Audio track {}: codec={}, lang={}, {}ch@{}Hz, {} kbps", track.getTrackNumber(),
                                codecId, track.getLanguage(), track.getChannels(), track.getSampleRateHz(), track.getBitrateKbps());
                        ...
                        log.debug("Subtitle track {}: codec={}, lang={}, format={}", track.getTrackNumber(),
                                codecId, track.getLanguage(), track.getSubtitleFormat());
                    }
    ...
    static StreamCodingType mapCodecId(String codecId) {
        if (codecId == null) throw new ParseException("Null codec ID");
    ...
    /**
     * Maps a Matroska subtitle codec ID to a short human-readable format string
     * stored in {@link SourceMediaInfo.SourceTrack#getSubtitleFormat()}.
     */

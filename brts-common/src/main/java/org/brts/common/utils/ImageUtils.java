...
public class ImageUtils {
    ...
    /**
     * Alpha multiplier applied when generating ghost images from the selected
     * state.
     */
    private static final float GHOST_OPACITY = 0.65f;

    /** Marker symbol stamped on the bottom-right corner of ghost images. */
    private static final String GHOST_MARKER = "\u25CC"; // ◌ (dotted circle — "placeholder")

    /**
     * Creates a "ghost" copy of the given image: every pixel's alpha is
     * multiplied by {@link #GHOST_OPACITY}, and a small marker symbol is
     * painted in the bottom-right corner to signal the image is synthetic.
     */
     ...
     * @return a new BufferedImage that is a copy of the source image with ARGB
     *         color model
    ...
    /**
     * Composes the overlay image onto the background image using the specified
     * transform, start point, and opacity.
     * 
     * @return the resulting composed image (same instance as background, modified
     *         in place)
     */
    public static BufferedImage compose(BufferedImage background, BufferedImage overlay, AffineTransform transform,
        ...
        log.debug("Composing image at ({}, {}) with opacity {} and transform {}. background size : {}x{}",
                x, y, opacity, transform, background.getWidth(), background.getHeight());

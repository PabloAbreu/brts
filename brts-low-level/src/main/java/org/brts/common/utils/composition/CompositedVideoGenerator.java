 ...
 *
 * <p>Outputs an H.264 M2TS video file (with the corresponding CLPI) by
 * compositing frames according to the provided {@link ImagesComposition}
 * configuration.
 *
 * <p>If the background image is a video, its audio streams are extracted and
 * muxed into the output M2TS file.
 *
 * <p>This class lives in {@code brt-low-level} so it can use
 * {@link M2tsWriter} and {@link ClipInfoWriter}.
 */
public class CompositedVideoGenerator {
    ...
    public static class Config {
         ...
         * When ≤ 0, the generator tries to derive the count from the base
         * video's frame count; an {@link IllegalArgumentException} is thrown
         * when the base is not a video and this value is not positive.
         */
        private int frameCount = -1;

        /**
         * Output frame rate in frames per second.
         * When ≤ 0, derived from the base video stream info or defaults to 24.0.
         */
        ...
        /**
         * Average MPEG-2 video target bitrate in kbps.
         * Defaults to 20 000 kbps (~20 Mbit/s), a safe value for Blu-ray.
         */
    ...
    /**
     * Generates a composited M2TS video file and the matching CLPI in
     * {@code outputDir}.
     *
     * <p>Steps performed:
     * <ol>
     ...
     */
    public void generate(ImagesComposition composition,
                         Path outputDir,
                         String clipName,
                         Config config,
        ...
        if (baseVideoPath != null && (fps <= 0 || frameCount <= 0)) {
            ...
            if (videoStream != null
                    && fps <= 0
                    && videoStream.getFrameRateFps() != null
            ...
            if (frameCount <= 0) {
                ...
                try {
                    ...
                    try { vf.close(); } catch (Exception ignored) {}
                }
            }
        }
        ...
        if (frameCount <= 0) {
            throw new IllegalArgumentException(
                    "frameCount must be explicitly set when the base image is not a video");
        }
        if (fps <= 0) {
            fps = 24.0;
        }

        log.info("Generating composited video '{}': {} frames @ {} fps ({}×{})",
                clipName, frameCount, fps, config.getWidth(), config.getHeight());
     ...
     * bytedeco/FFmpeg.
     *
     * <p>Each frame is composited via {@link CompositionBuffer}, converted from
     * BGR24 to YUV420P using {@code sws_scale}, then handed to the H.264 encoder.
     * The {@link MediaRepositoryImpl} is closed when encoding completes so that
     * any opened {@link M2tsVideoFrames} FFmpeg contexts are released promptly.
     */
    private void encodeToMpeg4(ImagesComposition composition,
                                int frameCount,
                                double fps,
                                Config config,
                                Path outputFile,
        ...
        try {
            ...
            // muxer can deliver each access unit before its DTS according to the PCR clock.
            ...
            codecCtx.max_b_frames(0); // B-frames reorder decode vs. display order; the raw ES muxer
                                      // assigns PTS sequentially and writes PTS-only PES headers, so
                                      // B-frames produce corrupt DTS and choppy playback.
            codecCtx.pix_fmt(AV_PIX_FMT_YUV420P);

            int ret = avcodec_open2(codecCtx, codec,
            ...
            try (OutputStream out = Files.newOutputStream(outputFile)) {
                ...
                for (int i = 0; i < frameCount; i++) {
                    ...
                    sws_scale(swsCtx,
                            bgrFrame.data(), bgrFrame.linesize(), 0, height,
                    ...
                    if (ret < 0) {
                        throw new IOException(
                                "avcodec_send_frame failed at frame " + i + ": " + ret);
                    }
    ...
    private static void drainPackets(AVCodecContext codecCtx,
                                     AVPacket packet,
    ...
    /**
     * Fills a pre-allocated BGR24 {@link AVFrame} from a {@link BufferedImage},
     * scaling the image to the frame's dimensions if necessary.
     */
    private static void fillBgrFrame(BufferedImage img,
                                     AVFrame bgrFrame,
                                     int width,
                                     int height) {
        ...
        ByteBuffer buf = bgrFrame.data(0)
                .position(0)
     ...
     *
     * <p>Output files are produced by {@link M2tsExtractor} and are named
     * {@code pid_<hex>.<ext>} inside a temporary subdirectory.
     */
    private void appendAudioEntries(String baseVideoPath,
                                    Path tempDir,
                                    List<M2tsDescriptor.StreamEntry> streams) throws IOException {
        ...
        for (M2tsStreamInfo as : audioStreams) {
            ...
            if (!Files.exists(esFile)) {
                log.warn("Audio ES file not found for PID 0x{}: {}",
                        Integer.toHexString(as.getPid()), esFile.getFileName());
                continue;
            }
            ...
            log.debug("  Added audio PID 0x{} ({})",
                    Integer.toHexString(as.getPid()), as.getCodingType());
        }
    }
    ...
    /**
     * Returns the video path of the base {@link ImageReference}, or {@code null}
     * if the base is a static image or synthetic.
     */
    private static String resolveBaseVideoPath(ImagesComposition composition, Path baseDir) {
        ...
        if (baseId != null && !baseId.isBlank()) {
            ref = images.stream()
                    .filter(r -> baseId.equals(r.getImageId()))
                    .findFirst()
                    .orElse(null);
        } else {
            ref = images.get(0);

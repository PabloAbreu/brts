...
public class MoviePlaylistParser implements BinaryParser<MoviePlaylist> {
     ...
     * <ol>
     *   <li><b>IG streams</b> — if any PlayItem carries an {@code INTERACTIVE_GRAPHICS} (0x91)
     *       stream in its STN, the playlist is interactive and therefore a menu.</li>
     ...
     *       If the playlist has ≥ 3 PlayItems and the most-repeated clip accounts for
     *       ≥ 80 % of all items, it is flagged as a menu.</li>
     * </ol>
     */
    private boolean detectMenu(MoviePlaylist playlist) {
        ...
        // 3. Heuristic: a playlist that repeats the same clip many times is likely a menu
    ...
    private List<PlayItemStream> readStn(BinaryReader r) throws IOException {
        ...
        for (int i = 0; i < totalStreams; i++) {
            ...
            if (type.isVideo()) {
                ...
                if (attrLength > 1) s.setLanguage(r.readAscii(3));
    ...
    private SubPath readSubPath(BinaryReader r) throws IOException {
        ...
        for (int i = 0; i < numSubPlayItems; i++) {
            ...
            r.skip(4 + 1 + 1 + 1); // codec_id, reserved, connection_condition, stc_id

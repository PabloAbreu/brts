...
public class IndexBdmvParser implements BinaryParser<IndexBdmv> {
    ...
    /**
     * Length of the BD-J object name field (5 ASCII characters),
     * located at byte offset 6 within each title entry.
     */
    ...
    private IndexBdmv readIndex(BinaryReader r) throws IOException {
        ...
        if (!MAGIC.equals(magic)) {
            throw new ParseException(
                    "Not an index.bdmv file: expected magic '" + MAGIC + "', got '" + magic + "'");
        }
        ...
        r.readUnsignedInt(); // ExtensionDataStartAddress — may be 0, not used during parsing
    ...
    /**
     * Reads the AppInfoBDMV section and returns the content_provider_name (trimmed of nulls).
     * The section structure is: section_length(4) + reserved(1) + flags(1) + name(32).
     */

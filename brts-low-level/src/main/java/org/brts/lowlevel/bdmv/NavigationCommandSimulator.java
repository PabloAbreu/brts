 ...
 * <p>
 * Internal engine commands (GOTO, SET, COMPARE, etc.) are fully executed with
 * an internal GPR register file. Commands that would have an external effect
 * (PLAY, JUMP, SET_SYSTEM, etc.) are traced but not executed.
 * <p>
 * Operand types (immediate vs register) are determined from the opcode's
 * immediate flags (bits 23 and 22) via {@link ParsedNavigationCommand}.
 */
public class NavigationCommandSimulator {
     ...
     * @param psrInit  initial PSR values (may be {@code null}); reading an uninitialised PSR at
     *                 runtime will throw {@link SimulationException}
     * @param maxSteps maximum number of instructions to execute before aborting
     */
    public NavigationCommandSimulator(List<NavigationCommand> commands,
                                      Map<Integer, Long> psrInit,
                                      long maxSteps) {
     ...
     * @param gprInit  initial GPR values (may be {@code null}); used to carry state
     *                 across chained simulations (e.g. CALL/RESUME flows)
     * @param maxSteps maximum number of instructions to execute before aborting
     */
    public NavigationCommandSimulator(List<NavigationCommand> commands,
                                      Map<Integer, Long> psrInit,
                                      Map<Integer, Long> gprInit,
                                      long maxSteps) {
    ...
    public record SimulationResult(
            List<String> externalEffects,
            Map<Integer, Long> finalGprState,
            int finalPc,
            long stepsExecuted,
    ...
    public SimulationResult run() {
        ...
        while (steps < maxSteps) {
            ...
            switch (m) {
                ...
                case SET_STREAM, SET_NV_TIMER, SET_BUTTON_PAGE,
                     ENABLE_BUTTON, DISABLE_BUTTON, SET_SEC_STREAM,
                     POPUP_OFF, STILL_ON, STILL_OFF,
                     SET_OUTPUT_MODE, SET_STREAM_SS -> {
                    traceExternal(m, cmd, steps);
                }
            }
        ...
        return new SimulationResult(
                Collections.unmodifiableList(new ArrayList<>(externalEffects)),
                getNonZeroGpr(),
                pc,
                steps,
                terminationReason,
    ...
    private long resolveOperand(NavigationCommand cmd, int operandIndex) {
        ...
        if (immediate) return raw & MASK_32;
        ...
        if ((operandValue & PSR_BIT) != 0) {
            ...
            if (psr[index] == null) {
                throw new SimulationException(
                        "PSR" + index + " was read but not provided in input (pc=" + pc + ")");
            }
            return psr[index];
        }
    ...
    private void storeResult(NavigationCommand cmd, int operandIndex, long value) {
        ...
        if (immediate) {
            throw new SimulationException(
                    "Cannot store result to immediate value (pc=" + pc + ")");
        }
        if ((raw & PSR_BIT) != 0) {
            throw new SimulationException(
                    "Cannot write to PSR from SET instruction (pc=" + pc + ")");
        }
    ...
    private NavigationCommandMnemonic resolveMnemonic(NavigationCommand cmd) {
        ...
        if (m != null) return m;
        throw new SimulationException(
                "Unknown command: " + parsed.getMnemonicString() + " (pc=" + pc + ")");
    }
    ...
    private String formatOperandValue(NavigationCommand cmd, int operandIndex) {
        try {
            ...
            boolean immediate = (operandIndex == 1) ? parsed.isOp1Immediate() : parsed.isOp2Immediate();
            if (immediate) return String.valueOf(resolved);
            if ((raw & PSR_BIT) != 0) return "PSR" + (raw & 0x7F) + "=" + resolved;
    ...
    private Long safeResolveOperand(NavigationCommand cmd, int operandIndex,
                                     NavigationCommandMnemonic m) {
        if (m.getOperandCount() < operandIndex) return null;
        try {

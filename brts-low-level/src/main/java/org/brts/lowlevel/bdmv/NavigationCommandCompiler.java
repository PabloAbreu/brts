 ...
 *
 * This file '/data/work/brts/brts-low-level/src/main/java/org/brts/lowlevel/bdmv/NavigationCommandCompiler.java' is part of BRTS.
 * ==============================
...
public final class NavigationCommandCompiler {
	...
	/**
	 * Returns the 32-bit opcode word for the given mnemonic, with the OperandCount (bits 31-29) and immediate-operand
	 * flags (bits 23 and 22) set according to the caller-specified operand kinds.
	 *
	 * @param mnemonic     symbolic name (case-insensitive), e.g. "PLAY_PL"
	 * @param op1Immediate {@code true} when operand 1 is an immediate (literal) value, {@code false} when it is a
	 *                     register reference (GPR/PSR)
	 * @param op2Immediate {@code true} when operand 2 is an immediate (literal) value, {@code false} when it is a
	 *                     register reference (GPR/PSR)
	...
	public static long compile(String mnemonic, boolean op1Immediate, boolean op2Immediate) {
		...
		if (cmd.getOperandCount() >= 1 && op1Immediate)
			base |= IMM_OP1;
		if (cmd.getOperandCount() >= 2 && op2Immediate)
			base |= IMM_OP2;
		return base;
	}

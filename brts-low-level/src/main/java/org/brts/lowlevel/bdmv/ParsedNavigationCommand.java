 ...
 * <p>
 * Stores the canonical three 32-bit words (opcode + two operands) and provides
 * rich accessors for the instruction fields, operand types, and human-friendly
 * descriptions.
 * <p>
 * This is the single authoritative representation for navigation commands,
 * bridging raw binary (12 bytes), symbolic form (mnemonic + operands), and
 * display.
 * <p>
 * Unlike the heuristic-based approach formerly used in
 * {@link NavigationCommandDecompiler}, operand types are determined from the
 * opcode's immediate flags (bits 23 and 22), which is the correct and
...
public final class ParsedNavigationCommand {
     ...
     */
    public static ParsedNavigationCommand compile(String mnemonic,
                                                   long op1, boolean op1Immediate,
     ...
     */
    public static ParsedNavigationCommand compile(NavigationCommandMnemonic mnemonic,
                                                   long op1, boolean op1Immediate,
                                                   long op2, boolean op2Immediate) {
    ...
    /**
     * Returns the {@link NavigationCommandMnemonic} for this command, or null
    ...
    public String getMnemonicString() {
        ...
        if (m != null) return m.getMnemonic();
    ...
    /**
     * Returns the {@link OperandKind} of operand 1 by combining the immediate
     * flag (bit 23) with the operand value's PSR/GPR encoding.
     */
    ...
    /**
     * Returns the {@link OperandKind} of operand 2 by combining the immediate
     * flag (bit 22) with the operand value's PSR/GPR encoding.
     */
    ...
    /**
     * Returns the register index for operand 1 when it is a register reference.
     * For PSR: bits 6:0. For GPR: bits 11:0.
     *
    ...
    /**
     * Returns the register index for operand 2 when it is a register reference.
     * For PSR: bits 6:0. For GPR: bits 11:0.
     *
    ...
    /**
     * Returns a human-friendly description of this command, using the correct
     * immediate flags from the opcode (not heuristic-based).
     */
    ...
    /**
     * Returns a human-friendly description of operand 1,
     * using the immediate flag from the opcode.
     */
    ...
    /**
     * Returns a human-friendly description of operand 2,
     * using the immediate flag from the opcode.
     */
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof ParsedNavigationCommand that)) return false;
    ...
    private static OperandKind operandKind(boolean immediate, long operandValue) {
        if (immediate) return OperandKind.IMMEDIATE;
        if ((operandValue & PSR_BIT) != 0) return OperandKind.PSR;
     ...
     * Fixes the heuristic bug in the old decompiler.
     */
    ...
    private String formatCommand(NavigationCommandMnemonic cmd) {
        return switch (cmd) {
            // No-operand commands
            case NOP, BREAK, RESUME, TERMINATE_PL, POPUP_OFF,
                 STILL_ON, STILL_OFF -> cmd.getDescription();
            ...
            case MOVE, SWAP, ADD, SUB, MUL, DIV, MOD,
                 RND, AND, OR, XOR, BITSET, BITCLR, SHL, SHR ->
                    formatBinaryOperation(cmd, describeOperand1(), describeOperand2());

            // Comparison operations
            case BC, EQ, NE, GE, GT, LE, LT ->
                    formatComparison(cmd, describeOperand1(), describeOperand2());
    ...
    /**
     * Returns a friendly name for well-known Player Status Registers.
     * Reference: libbluray mobj_print.c psr_info[].
     */

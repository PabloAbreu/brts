 ...
 * </pre>
 * The {@link #opcode} field stores the base opcode <em>without</em> OperandCount (bits 31-29)
 * and <em>without</em> immediate flags (bits 23-22). These are added by the compiler and
 * stripped by the decompiler.
 */
@Getter
public enum NavigationCommandMnemonic {
    ...
    // "jump" plays the given title and does not return, while "call" plays the title and returns to the next command when done
    ...
    // ────────────────────────────────────────────────────────────────────────
    // play a playlist by id (name of the .mpls file without the extension, e.g. 1 for "00001.mpls")
    ...
    /**
     * Base opcode value (without OperandCount in bits 31-29 and without
     * ImmediateFlags in bits 23-22). Encodes CommandGroup, CommandSubGroup,
     * and the relevant option field (BranchOption, CompareOption, or SetOption).
     */

 ...
 *
 * This file '/data/work/brts/brts-low-level/src/main/java/org/brts/lowlevel/bdmv/BdjoWriter.java' is part of BRTS.
 * ==============================
...
public class BdjoWriter implements BlurayFileWriter<Bdjo> {
	...
	public void write(Bdjo model, OutputStream output) throws IOException {
		...
		String version = model.getVersion() != null && !model.getVersion().isBlank() ? model.getVersion()
				: DEFAULT_VERSION;
	...
	private void writeAppEntry(ByteArrayBinaryWriter w, Bdjo.BdjoApp app) throws IOException {
		...
		// Format: 2 bytes (0x0000) + 4 bytes descriptor_length (body + 4) + 4 bytes
		// (0x00000000)
	...
	private void writeAppParams(ByteArrayBinaryWriter w, List<String> params) throws IOException {
		...
		// Word-align: if data_length is even, total (1+data_length) is odd, need 1 pad
		// byte
		if ((dataLength & 1) == 0) {
			w.writePadding(1);
		}

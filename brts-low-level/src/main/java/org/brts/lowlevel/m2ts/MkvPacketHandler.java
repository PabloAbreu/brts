...
/**
 * {@link M2tsPacketHandler} that writes demuxed elementary streams into a
 * Matroska (MKV) container using the jebml library.
 * <p>
 * Each tracked PID is mapped to an MKV track. The handler accumulates PES
 * payload bytes per PID and flushes a complete <em>access unit</em>
 * (video frame / audio frame / subtitle display set) to the MKV writer
 * whenever a new Payload Unit Start is received for the same PID.
 *
 * <h2>Codec mapping</h2>
 * <ul>
...
public class MkvPacketHandler implements M2tsPacketHandler {
    ...
    public MkvPacketHandler(M2tsInfo info, Path mkvPath, Set<Integer> pidFilter) throws IOException {
        ...
        for (M2tsStreamInfo s : streams) {
            ...
            if (track == null) {
                log.warn("Unsupported coding type {} for PID 0x{}, skipping",
                        s.getCodingType(), Integer.toHexString(s.getPid()));
                continue;
            }
            ...
            log.info("  MKV track {} ← PID 0x{} [{}] codec={}",
                    trackNo, Integer.toHexString(s.getPid()),
    ...
    /**
     * Writes Matroska chapter entries derived from the given playlist marks.
     * Must be called after demuxing (so that {@code basePts} is known) and
     * before {@link #close()}.
     *
    ...
    public void writeChapters(List<PlayMark> marks) {
        ...
        for (int i = 0; i < marks.size(); i++) {
            ...
            if (chapterTimeNs < 0) chapterTimeNs = 0;

 ...
 *
 * This file '/data/work/brts/brts-low-level/src/main/java/org/brts/lowlevel/m2ts/PlaylistToMkvExtractor.java' is part of BRTS.
 * ==============================
...
public class PlaylistToMkvExtractor {
	...
	public void extract(Path streamDir, MoviePlaylist playlist, Path outputMkv, Set<Integer> audioPids,
		...
		try (MkvPacketHandler handler = new MkvPacketHandler(filteredInfo, outputMkv, selectedPids)) {
			...
			// Write chapter marks into the MKV (must happen after demux so basePts is
			// known)
	...
	/**
	 * Builds the set of PIDs to include: all video + selected audio + selected subtitles.
	 */
	private Set<Integer> resolveSelectedPids(PlayItem playItem, Set<Integer> audioPids, Set<Integer> subtitlePids) {

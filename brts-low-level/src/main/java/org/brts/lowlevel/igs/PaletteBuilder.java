...
/**
 * Builds an {@link IgsPalette} from one or more ARGB {@link BufferedImage}s
 * by collecting unique colours and quantising down to ≤ 256 entries.
 * <p>
...
public final class PaletteBuilder {
     ...
     * <p>
     * If there are more than 255 unique colours (index 0 is reserved for
     * transparent), a simple popularity-based quantisation is used.
     *
    ...
    /**
     * Builds a palette from an explicit list of ARGB colours.
     * Index 0 is always transparent. The colours are assigned indices 1..N.
     *
     * @param paletteId the palette id to assign
    ...
    public static IgsPalette buildFromColors(int paletteId, int... colors) {
        ...
        for (int argb : colors) {
            if (nextId >= MAX_ENTRIES) break;
            if (argb == 0x00000000) continue; // skip transparent, already added
            if (!seen.add(argb)) continue;     // skip duplicates

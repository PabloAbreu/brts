...
/**
 * IGS Muxer — reassembles extracted IGS resources (from {@link IgsDemuxer})
 * back into a single raw IGS elementary stream file.
 * <p>
 * The muxer reads the manifest and per-display-set JSON + RLE files, re-encodes
 * each segment in binary form, and concatenates them into a single {@code .igs}
 * file identical to what {@link org.brts.common.m2ts.FilePacketHandler} would
 * produce during extraction.
 *
...
public class IgsMuxer {
    ...
    /**
     * Encodes a fully populated {@link IgsDisplaySet} (built in memory) into
     * raw IGS elementary stream bytes.
     * <p>
     * Segment order follows the Blu-ray spec:
     * ICS → PDS → WDS → ODS → END_OF_DISPLAY.
     *
    ...
    public byte[] encodeObject(IgsObject obj) {
        ...
        if (firstInSeq) {
            int dataLength = obj.getDataLength() > 0
                    ? obj.getDataLength()
                    : rleData.length + 4; // 4 = width(2) + height(2)
            // dataLength (24 bits)
    ...
    public byte[] encodeObject(IgsDemuxer.ObjectMetadata meta, byte[] rleData) {
        boolean firstInSeq = meta.getSequenceDescriptor() != null
                && meta.getSequenceDescriptor().isFirstInSequence();
        boolean lastInSeq = meta.getSequenceDescriptor() != null
                && meta.getSequenceDescriptor().isLastInSequence();
    ...
    public byte[] encodeIcs(IgsCompositionSegment ics) throws IOException {
        ...
        int seqByte = 0;
        if (sd != null) {
            if (sd.isFirstInSequence()) seqByte |= 0x80;
            if (sd.isLastInSequence()) seqByte |= 0x40;
        }
        buf.write(seqByte);

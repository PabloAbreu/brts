...
/**
 * Renders subtitle text into ARGB {@link BufferedImage}s suitable for
 * PGS Object Definition Segments.
 * <p>
 ...
 * <ol>
 *   <li>Strip HTML tags from text and build an {@link AttributedString}
 *       with bold/italic/underline ranges.</li>
 *   <li>Measure the text bounding box.</li>
 *   <li>Draw outline (thick stroke in outline colour) then fill
 *       (text colour) for legibility.</li>
 * </ol>
 *
 * @see PgsRenderConfig
 */
public class PgsSubtitleRenderer {
    ...
    private static final Logger log = LoggerFactory.getLogger(PgsSubtitleRenderer.class);
     ...
     *
     * @param text     the subtitle text (may contain {@code <b>}, {@code <i>},
     *                 {@code <u>} tags and newlines)
    ...
    public RenderResult render(String text, SubtitlePosition position) {
        ...
        for (String line : lines) {
            ...
            AttributedString attrLine = buildAttributedLine(
                    line, charOffset, parsed.ranges, baseFont);

            TextLayout layout = new TextLayout(
                    attrLine.getIterator(), gMeasure.getFontRenderContext());
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            TextLayout layout = new TextLayout(
                    attrLine.getIterator(), g.getFontRenderContext());

            float xPos = outlinePad + (maxWidth - meas.width) / 2.0f;
            yPos += layout.getAscent();

            // Draw outline (thick stroke)
            Shape textShape = layout.getOutline(
                    AffineTransform.getTranslateInstance(xPos, yPos));
            g.setColor(new Color(config.getOutlineColor(), true));
            g.setStroke(new BasicStroke(config.getOutlineWidth(),
    ...
    static ParsedText parseHtmlTags(String text) {
        ...
        if (boldStart >= 0)      ranges.add(new StyleRange(boldStart, end, StyleRange.Type.BOLD));
        if (italicStart >= 0)    ranges.add(new StyleRange(italicStart, end, StyleRange.Type.ITALIC));
        if (underlineStart >= 0) ranges.add(new StyleRange(underlineStart, end, StyleRange.Type.UNDERLINE));

        return new ParsedText(plain.toString(), ranges);
    }

    /**
     * Builds an {@link AttributedString} for a single line of text,
     * applying style ranges that overlap this line's character range
     * within the full plain text.
     */
    private AttributedString buildAttributedLine(
            String line, int lineStartInFull, List<StyleRange> allRanges, Font baseFont) {
    ...
    record StyleRange(int start, int end, Type type) {
        enum Type { BOLD, ITALIC, UNDERLINE }
    }

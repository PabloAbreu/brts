...
/**
 * Round-trip tests for {@code .bdjo}: write via {@link BdjoWriter},
 * re-parse via {@link BdjoParser}, and verify the model is preserved.
 */
class BdjoRoundTripTest {
    ...
    void roundTrip_accessiblePlaylists_withNames() throws Exception {
        ...
        Bdjo reparsed = roundTrip(original);
        assertThat(reparsed.getAccessiblePlaylists().getPlaylistNames())
    ...
    void roundTrip_appWithParameters() throws Exception {
        ...
        Bdjo reparsed = roundTrip(original);
        assertThat(reparsed.getApplications().get(0).getParameters())
    ...
    private Bdjo buildFullBdjo() {
        ...
        bdjo.setApplications(new ArrayList<>(List.of(
                buildApp(2, "StandardMenuXlet", "com.dis.radius.StandardMenuXlet"),

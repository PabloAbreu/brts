...
/**
 * Extended ClipInfo round-trip tests covering multiple codec types,
 * PG/IG streams, timing boundaries, and application types.
 */
class ClipInfoExtendedRoundTripTest {
    ...
    void roundTrip_dtsHdMasterAudio_codecAndLanguagePreserved() throws Exception {
        ClipInfo ci = build(StreamCodingType.H264_AVC, 0x06, 0x04, 0x03,
                StreamCodingType.DTS_HD_MASTER_AUDIO, 0x0C, 0x04, "jpn");
        ClipInfo out = roundTrip(ci);
    ...
    void roundTrip_ac3PlusAudio_preserved() throws Exception {
        ClipInfo ci = build(StreamCodingType.H264_AVC, 0x06, 0x04, 0x03,
                StreamCodingType.DOLBY_AC3_PLUS, 0x06, 0x01, "deu");
        ClipInfo out = roundTrip(ci);
    ...
    void roundTrip_longDuration_timingWithinOneTick() throws Exception {
        ...
        assertThat(out.getDuration().getTicks())
                .isCloseTo(648_000_000L, offset(2L));
        assertThat(out.getTsRecordingEndPts().getTicks())
                .isCloseTo(648_000_000L, offset(2L));
    }
    ...
    void roundTrip_nonZeroStartPts_preserved() throws Exception {
        ...
        assertThat(out.getTsRecordingStartPts().getTicks())
                .isCloseTo(startTicks, offset(2L));
        assertThat(out.getTsRecordingEndPts().getTicks())
                .isCloseTo(endTicks, offset(2L));
    }
    ...
    void roundTrip_multipleAudioTracks_allPreserved() throws Exception {
        ...
        assertThat(out.getStreams().stream().filter(s -> s.getLanguage() != null && s.getLanguage().equals("fra")).count())
                .isEqualTo(1);
    }
    ...
    private ClipInfo build(StreamCodingType videoCodec, int vFormat, int vFrameRate, int vAspect,
        ...
        if (audioCodec != null) streams.add(audioStreamOf(audioCodec, 0x1100, aCh, aSr, aLang));

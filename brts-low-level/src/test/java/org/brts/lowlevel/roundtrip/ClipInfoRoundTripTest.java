...
import org.brts.lowlevel.model.clpi.*;
...
class ClipInfoRoundTripTest {
    ...
    void roundTrip_basicClipInfo_fieldsAreConsistent() throws Exception {
        ...
        assertThat(reparsed.getDuration().getTicks())
                .isCloseTo(original.getDuration().getTicks(), org.assertj.core.data.Offset.offset(2L));

        // Verify streams
        assertThat(reparsed.getStreams()).hasSize(original.getStreams().size());
        ClipStream videoOut = reparsed.getStreams().stream()
        ...
        ClipStream audioOut = reparsed.getStreams().stream()

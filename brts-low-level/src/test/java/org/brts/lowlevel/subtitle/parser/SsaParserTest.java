...
class SsaParserTest {
    ...
    void convertSsaToHtml() {
        assertEquals("Normal <b>bold</b> end",
                SsaParser.convertSsaToHtml("Normal {\\b1}bold{\\b0} end"));

        assertEquals("before <i>italic</i> after",
                SsaParser.convertSsaToHtml("before {\\i1}italic{\\i0} after"));

        assertEquals("line1\nline2",
                SsaParser.convertSsaToHtml("line1\\Nline2"));
    }

    @Test
    void stripsUnknownOverrides() {
        assertEquals("Clean text",
                SsaParser.convertSsaToHtml("{\\fad(500,200)}Clean text"));
    }
